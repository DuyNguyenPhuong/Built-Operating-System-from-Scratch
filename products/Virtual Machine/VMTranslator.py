...
def addASM():
    addASMarray = ["@SP", 'M=M-1', 'A=M-1', 'D=M',
                   '@SP', 'A=M', 'D=D+M', '@SP', 'A=M-1', 'M=D']
...
def subASM():
    subASMarray = ["@SP", 'M=M-1', 'A=M-1', 'D=M',
                   '@SP', 'A=M', 'D=D-M', '@SP', 'A=M-1', 'M=D']
...
def popASM(s, name):
    ...
    if arg1 != 'static':
        popASMarray = ["@SP", 'A=M-1', 'D=M',
                       '@R15', 'M=D', '@'+str(arg2), 'D=A']
    ...
    if arg1 == 'pointer' or arg1 == 'temp':
        ...
        printarray(['@R14', 'M=D', '@R15', 'D=M',
                   '@R14', 'A=M', 'M=D', '@SP', 'M=M-1'])
    elif arg1 == 'static':
        printarray(['@SP', 'M=M-1'])
    else:
...
def writeCall(functionName, numArgs, i):
    ...
    printarray(['@'+'return-address'+str(i), 'D=A',
               "@SP", 'A=M', 'M=D', '@SP', 'M=M+1'])
    pushM('LCL')
    pushM('ARG')
    pushM('THIS')
